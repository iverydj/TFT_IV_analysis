...
if ConditionData:
    ...
    if len(condition_list) != len(data_names):
        raise ValueError('condition data miss match')
...
for i in range(len(data_names)):
    ...
    try :
        ...
        if CalculateVth or CalculateSS or CalculateMobility:
            ...
            subthreshold_indices = np.where(np.log10(d_log_I_d_dV_g_fine) +1e-9 >= log_threshold_findSS)[0]
        ...
        if CalculateMobility:
            ...
            g_m = np.gradient(I_d_rough, V_g_rough)  # dI_d/dV_g
        ...
        if CalculateSS:
            axs[1, 0].plot(V_g_fine, d_log_I_d_dV_g_fine, label=r'$\log_{10}(I_d)$', color='purple')
            axs[1, 0].set_xlabel("Gate Voltage ($V_g$) [V]")
            axs[1, 0].set_ylabel(r"$\log_{10}(I_d)$")
        ...
        if CalculateMobility:
            ...
            axs[2, 0].set_ylabel("Mobility ($\mu$) [m^2/V·s]")
            ...
            axs[2, 1].set_ylabel("Mobility ($\mu$) [m^2/V·s]")
            ...
            axs[2, 2].set_ylabel("Mobility ($\mu$) [m^2/V·s]")
